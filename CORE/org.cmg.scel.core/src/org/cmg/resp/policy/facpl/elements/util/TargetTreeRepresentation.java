package org.cmg.resp.policy.facpl.elements.util;
...
public class TargetTreeRepresentation {
	...
	public MatchDecision getDecisionValue(AuthorizationRequest request){
		MatchDecision decision;
		if (root instanceof TargetExpression){
			//only one expression
			try{
				decision = ((TargetExpression)root).evaluateTarget(
						request.getAttributeValue(((TargetExpression)root).getStruct_name()));
			}catch (Throwable e){
				if (e instanceof MissingAttributeException){
					//we assume that in case of absence of the attribute the policy is no-match 	
			...
			for (TargetTreeRepresentation tr : this.child) {
				decisions.add(tr.getDecisionValue(request));
			}
